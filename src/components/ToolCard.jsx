...
      <div className={`
        relative bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex transition-all duration-200 hover:shadow-md hover:border-purple-200
        ${viewMode === 'list' ? 'flex-row items-center gap-6' : 'flex-col h-full'}
      `}>
        <img
          src={tool.image}
          alt={tool.name}
        ...
        />
        <div className={`
          ${viewMode === 'list' ? 'flex-grow' : 'flex flex-col text-center flex-grow'}
        `}>
          <h3 className="text-xl font-semibold text-gray-900 mb-2 group-hover:text-purple-600 truncate" title={tool.name}>
            {tool.name}
          </h3>
