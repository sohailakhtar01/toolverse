...
async function ultraSmartSearch(query, analysis) {
  ...
  const results = await Tool.find(mongoQuery.length > 0 ? mongoQuery : { $or: searchConditions })
    ...
    .limit(10)
    .select('displayName name shortDescription longDescription description categories pricingType slug rating tags features overview keywords')
    .lean();
  ...
  if (results.length === 0) {
    ...
    if (categoryKeywords.length > 0) {
      ...
      const broadResults = await Tool.find({ $or: broadQuery })
        ...
        .limit(8)
        .select('displayName name shortDescription longDescription categories pricingType slug rating tags')
        .lean();
...
export async function POST(request) {
  try {
    ...
    // 🧠 ANALYZE with advanced NLP
    const analysis = analyzeQuery(searchQuery);
    
    // 🔍 ULTRA-SMART SEARCH across all DB fields
    const searchResults = await ultraSmartSearch(searchQuery, analysis);
    ...
    const formattedTools = searchResults.map(tool => ({
      ...
      description: tool.shortDescription || tool.longDescription?.substring(0, 180) || tool.description?.substring(0, 180) || tool.overview?.substring(0, 180) || 'Explore this amazing AI tool',
